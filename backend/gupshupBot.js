...
const handleMessage = async (from, message, mediaUrl) => {
  ...
  switch (state.step) {
    case 'start':
      ...
      response = 'Welcome to the Complaint Redressal System! Let’s file your complaint. What’s your name?';
      break;
...
const submitComplaint = async (from, data) => {
  try {
    ...
    if (data.photoUrl) {
      // Download photo from Gupshup media URL
...
const sendMessage = async (to, message) => {
  try {
    ...
    console.error('Gupshup send message error:', error.message);
  }
};
